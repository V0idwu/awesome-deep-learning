...
def multibox_prior(data, sizes, ratios):
    ...
    w = (
        torch.cat((size_tensor * torch.sqrt(ratio_tensor[0]), sizes[0] * torch.sqrt(ratio_tensor[1:]))) * in_height / in_width
...
def batchify(data):
    ...
    return (d2l.reshape(d2l.tensor(centers), (-1, 1)), d2l.tensor(contexts_negatives), d2l.tensor(masks), d2l.tensor(labels))
    ...
    def update(self, config: dict, error: float, info=None):
        ...
        if ri < self.r_max:
            ...
            if len(self.observed_error_at_rungs[ri]) >= ni:
                ...
                self.queue = [dict(config, max_epochs=riplus1) for config in best_performing_configurations] + self.queue
        ...
        for y in range(4):
            for x in range(4):
                ...
                if env_desc[y, x].decode() == "H":
                    ax.text(x, y, str(env_desc[y, x].decode()), ha="center", va="center", color="y", size=20, fontweight="bold")

                elif env_desc[y, x].decode() == "G":
                    ax.text(x, y, str(env_desc[y, x].decode()), ha="center", va="center", color="w", size=20, fontweight="bold")

                else:
                    ax.text(x, y, str(env_desc[y, x].decode()), ha="center", va="center", color="g", size=15, fontweight="bold")
